...
\[
\begin{array}{rcl}
Y_k &\sim& N(\mu + \theta_k + \beta_kX, \sigma_k^2)\\
\theta_k &\sim& N(0, \tau^2)
\end{array}
\]

And, if $\beta_k$ are random, these, too, would have distributions.
...
Now, this is for an indirect comparison between two treatments, $i$ and $j$. So, what about 
...
<!-- An immediate question with this dataset of only two treatments, thus no indirect comparison, is to compare the results of the network meta-analysis with those of a standard meta-analysis.  -->

<!-- ```{r} -->
<!-- # how to rma on binomial count data  -->
<!-- # set measure = "OR" for log odds ratio -->
<!-- rma_dat <-  -->
<!-- bcg_vaccine %>%  -->
<!--   select(-trtn) %>%  -->
<!--   pivot_wider(id_cols = studyn,  -->
<!--               names_from = trtc, -->
<!--               names_glue ="{trtc}_{.value}",  -->
<!--               values_from = c(latitude, r, n)) -->
...
<!-- ```{r} -->
<!-- rma_dat %>%  -->
<!--   escalc(x1i = Unvaccinated_r, x2i = Vaccinated_r, n1i = Unvaccinated_n, n2i = Vaccinated_n, measure = "OR", data = ., slab = studyn) -->
...
<!-- Threshold analysis is a technique proposed for the interpretation of meta-analytic results. In this section, we explore threshold analysis with the `nmathresh::` package.  -->
...
In this case, we have a continuous response variable.

## Network meta-analysis model

We have five treatments of interest for our response measure, 

> the mean off-time reduction in patients given dopamine agonists as adjunct therapy in Parkinson's disease from 7 trials comparing four active drugs and placebo [`?parkinsons` documentation, @phillippo_multilevelnetworkmetaregression_2020, todo Dias citation].

Since `multinma::` provides tools for Bayesian network meta-analysis, we have the following model structure
$$
\boldsymbol y_{.jk} = \pi_\text{Agg}(\boldsymbol\theta_{.jk}),\\
g(\boldsymbol\theta_{.jk}) = \boldsymbol\mu_j + \boldsymbol\delta_{jk}
$$

where

- The subscript '$\cdot$' denotes an aggregate-level value.
- $\pi_\text{Agg}(\theta_{.jk})$ denotes "suitable" likelhood for aggregate value $\theta_{.jk}$
- $\theta_{.jk}$ is the expected summary outcome of treatment $k$ in the study $k$
- the expected summary is transformed by the link function $g(\cdot)$
- $\boldsymbol \mu$
- 

Now to apply this structure to the data we have in `parkinsons`.

For an arm-based network meta-analysis, begin by converting the raw data to a network object. 

Turning to Rethinking 
...
The number of studies measure must be relational. 
